...
void alice_salary(const int year, const int month) {
    int skip_salary_month = 1;
    int premium_month = 12;
    int dismissal_month = 5;
    int dismissal_year = 2030;
    int new_work_month = 7;
    int new_work_year = 2030;
    if (month == skip_salary_month) {

        return;
    }
    if (month == premium_month) {
        alice.bank_account += alice.salary;
    }
    if (month == dismissal_month and year == dismissal_year) {
        alice.old_salary = alice.salary;
        alice.salary = 0;
    }
    if (month == new_work_month and year == new_work_year) {
     
        alice.salary = alice.old_salary*2;
    }
    alice.bank_account += alice.salary;
    if (month == 12) {
