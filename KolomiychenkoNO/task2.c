...
void matrix_error(MatrixException error_level, const char *message) {
...
Matrix matrix_allocate(const Matrix frame) {
    ...
    if (frame.rows > 0 && frame.cols > 0 && SIZE_MAX / frame.rows / frame.cols < sizeof(double)) {
        matrix_error(ERROR, "Memory allocation failed: size_t overflow.\n");
        return MATRIX_ZERO;
    }

    Matrix matrix = {frame.rows, frame.cols, malloc((frame.rows * frame.cols) * sizeof(double))};
    if (matrix.data == NULL) {
...
void matrix_pass_array(Matrix *matrix, double array_name[]) {
    if (array_name == NULL) {
        matrix_error(ERROR, "NULL array passed.\n");
        return;
    }
...
Matrix matrix_identity(const size_t rows, const size_t cols) {
    ...
    memset(identity_matrix.data, 0, rows * cols * sizeof(double));

    for (size_t idx = 0; idx < rows; idx++) {
        identity_matrix.data[idx * (cols + 1)] = 1.0;
    }
