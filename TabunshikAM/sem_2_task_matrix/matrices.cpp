...
#define MAKE_MATRIX(var, r, c) Matrix var(r, c, #var)
#define MAKE_RESULT_MATRIX(var, expr) Matrix var = (expr); var.set_name(#var)
...
public:
    Matrix(size_t des_rows, size_t des_cols, const char* matrix_name = nullptr); // Конструктор
    ~Matrix(); // Деструктор
    Matrix(const Matrix& other); // Конструктор копирования
    Matrix(Matrix&& other); // Конструктор переноса

public:  // Сервисные методы (функции)
    void set_name(const char* matrix_name) { name = matrix_name; }
    void print();
    void set_zeros();
    void set_identity();
    void fill_with_indices();
    void fill_with_random(double min, double max);
...
Matrix::Matrix(size_t des_rows, size_t des_cols, const char* matrix_name)
{
    rows = des_rows;
    cols = des_cols;
    name = matrix_name;
...
void Matrix::fill_with_random(double min, double max) // Заполнение матрицы индексами элементов
{
...
int main()
{
    ...
    printf("above");
    MAKE_RESULT_MATRIX(F, D * E);
    F.print();
    printf("end");

    MAKE_RESULT_MATRIX(G, F.transp());
    G.print();

    printf("Matrix_pow test\n");
    B.print();

    MAKE_RESULT_MATRIX(J, B ^ 2);
    J.print();
