...
void alice_init()
{
    ...
    alice.account -= alice.mortgage.first_payment;
...
{
    alice.account -= (alice.mortgage.monthly_payment + alice.house_bills);
}


void bob_house(const int month)
{
    bob.account -= (bob.rent + bob.house_bills);
...
{
    alice.account -= (alice.household.food + alice.household.person_expens);
...
{
    bob.account -= (bob.household.food + bob.household.person_expens);
