...
Matrix matrix_umnok(const Matrix A, const double k)
{
    ...
    memcpy(C.data, A.data, sizeof(double) * A.cols * A.rows);
...
double matrix_det(const Matrix A)
{
    ...
    matrix_exception(WARNING, "Нельза посчитать больше 3х3");
    return NAN;
}
   ...
   Matrix C = matrix_allocate(A.cols, A.rows);
     for (size_t row = 0; row < C.rows; row++){
        for (size_t col =0; col < C.cols; col++){
            if (C.cols = C.rows){
                C.data[col * C.cols + col] = 1;
            }
            else {
                C.data[col * C.cols + row] = 0;
            }
        }
    }
    return C;
...
Matrix matrix_copy(const Matrix A)
{
    ...
    if((copy.cols != A.cols) || (copy.rows != A.rows)){
    	Matrix_exception(ERROR, " ");
    	return NULL_MATRIX;
	}
...
Matrix matrix_exp(const Matrix A, int n)
{
    ...
    Matrix E = matrix_one(A);

    if (E.data == NULL) return NULL_MATRIX;
