...
std::vector<BatchCommand> parseBatchCommands(const std::string& input) {
    ...
    while (std::getline(ss, token, ';')) {
        ...
        if (colonPos == std::string::npos) {
            std::cout << "[BATCH PARSER] No colon found, skipping" << std::endl;
            continue;
        }
        ...
        if (direction == "forward" || direction == "fwd" || direction == "fw" || direction == "f") {
            ...
            std::cout << "[BATCH PARSER] Unknown direction: " << direction << std::endl;
        }
  ...
  while (true) {
    ...
    if (n < 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            // Можно отправить keep-alive или просто продолжить
    ...
    for (int i = 0; i < n; i++) {
      ...
      if ((c == 'w' || c == 'a' || c == 's' || c == 'd' || c == ' ' || c == 'l') && !inBatchMode) {
        if (!lineBuffer.empty()) {
...
int main() {
  ...
  std::cout << "=====================================\n";
  std::cout << "  ROBOT CONTROL SERVER (MULTI-CLIENT)\n";
