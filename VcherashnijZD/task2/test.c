...
#define ASSERT_MATRIX_EQ(A, B, accuracy) \
    do {                                  \
        int are_equal;                     \
        STATUS status = matrix_equals(A, B, accuracy, &are_equal); \
...
Matrix* generate_random_matrix(int rows, int cols) {
    Matrix* matrix;
    STATUS status = matrix_alloc(rows, cols, &matrix);
    if (status != OK) {
...
void test_matrix_identity() {
    Matrix* matrix;
    STATUS status = matrix_alloc(3, 3, &matrix);
    ASSERT_STATUS_OK(status);
...
void test_matrix_fill_val() {
    Matrix* matrix;
    STATUS status = matrix_alloc(2, 3, &matrix);
    ASSERT_STATUS_OK(status);
...
void test_matrix_clone() {
    ...
    Matrix* clone = NULL;
    STATUS status = matrix_clone(original, &clone);
    ASSERT_STATUS_OK(status);
...
void test_matrix_mult() {
    ...
    Matrix* result = NULL;
    STATUS status = matrix_mult(matA, matB, &result);
    ASSERT_STATUS_OK(status);
...
void test_matrix_mult_by_num() {
    ...
    STATUS status = matrix_mult_by_num(a, mat);
    ASSERT_STATUS_OK(status);
...
void test_matrix_det() {
    Matrix* mat = generate_random_matrix(3, 3);
    double det_a;
    STATUS status = matrix_det(mat, &det_a);
    ASSERT_STATUS_OK(status);
...
void test_matrix_pow() {
    Matrix* mat = generate_random_matrix(3, 3);
    Matrix* result = NULL;
    STATUS status = matrix_pow(mat, 2, &result);
    ASSERT_STATUS_OK(status);
    matrix_free(mat);
    matrix_free(result);
}
...
void test_matrix_check_max_diff() {
    ...
    Matrix* matB = generate_random_matrix(3, 3);
    double max_diff;
    STATUS status = matrix_check_max_diff(matA, matB, &max_diff);
    ASSERT_STATUS_OK(status);
...
void test_matrix_exp() {
    Matrix* mat = generate_random_matrix(3, 3);
    Matrix* result = NULL;
    STATUS status = matrix_exp(mat, &result);
    ASSERT_STATUS_OK(status);
...
void test_matrix_equals() {
    ...
    STATUS status = matrix_equals(matA, matA, EQUAL_TEST_ACCURACY, &are_equal);
    ASSERT_STATUS_OK(status);
    ASSERT_DOUBLE_EQ(1.0, are_equal, 0.0);
    // Test for non-equal matrices
    status = matrix_equals(matA, matB, EQUAL_TEST_ACCURACY, &are_equal);
    ASSERT_STATUS_OK(status);
