...
void alice_init() {
	...
	alice.mortgage.rate = 0.07;
	alice.mortgage.monthly_payments = 93142; 	// https://calcus.ru/kalkulyator-ipoteki?input=eyJjdXJyZW5jeSI6IlJVQiIsInR5cGUiOiIxIiwiY29zdCI6IjE1MDAwMDAwIiwic3RhcnRfc3VtIjoiMTAwMDAwMCIsInN0YXJ0X3N1bV90eXBlIjoiMSIsInBlcmlvZCI6IjMwIiwicGVyaW9kX3R5cGUiOiJZIiwicGVyY2VudCI6IjciLCJwYXltZW50X3R5cGUiOiIxIn0=
...
void alice_cat(const int month, const year) {
...
alice_car(const int month, const int year) {
	int car_year = 0;
...
bob_car(const int month, const int year) {
	int car_year = 0;
