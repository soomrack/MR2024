...
Person year_inflation (Person person) 
{
    ...
    person.apartment_coast *= 1.07; //учет инфляции в стоимости жилья
    return person;
}
...
Person month_capital_changes (Person person)
{
	person.capital = person.capital + person.salary - person.monthly_expences - person.person_flat_payment;  //ежем. доходы/расходы
	person.capital *= 1.016;  //учет банковского вклада
	return person;
}


Person date_check (Person person)
{
	now_date.month++;
...
Person simulation (Person person) 
{
    ...
    while ((now_date.year < last_date.year) || (now_date.month < last_date.month)) {
       	...
       	person = month_capital_changes(person);
		person = date_check(person);
    }
    return person;
}
