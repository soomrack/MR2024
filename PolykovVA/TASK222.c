...
{
    if (A.rows != A.cols) {
        matrix_exception(WARNING, "Для вычисления определителя матрица должна быть квадратной.");
        return NAN;
    }
    ...
    matrix_exception(WARNING, "Расчет определителя для этого размера не реализован.");
    return 0;
}
