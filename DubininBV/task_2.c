...
void matrix_mult_coef(const Matrix A, const double coeff)
{
	if (A.data == NULL) {
...
Matrix matrix_exp(const Matrix A, const size_t n)
{
    ...
    for (size_t idx = 1; idx <= n; idx++) {
        ...
        matrix_mult_coef(matrix_exp_temp, coeff_to_multy);
...
Matrix matrix_inverse(Matrix A)
{
    ...
    matrix_mult_coef(transp_cofactor_matrix, 1 / det_A);
...
void calculations()
{
    ...
    matrix_fill(A, (double[]) {1,2,3,4,5,6,7,8,9});
    ...
    printf("Скопированная матрица B \n");
    matrix_print(B);
