...
Actuator g_heater(PIN_HEAT_CTRL);
Actuator g_vent(PIN_VENT_CTRL);
Actuator g_pump(PIN_PUMP_CTRL);
Actuator g_light(PIN_LIGHT_CTRL);
...
void control_air_temp(Climate& config, Termometer& sensor, WorkTime& work_time, Actuator& heater, Actuator& vent) {
...
void control_air_hum(Climate& config, Hygrometer& sensor, WorkTime& work_time, Actuator& vent) {
    if (work_time.is_work_time()) {
...
void control_soil_hum(Climate& config, SoilHygrometer& sensor, WorkTime& work_time, Actuator& pump) {
    if (work_time.is_work_time()) {
...
void plan_vent(Climate& config, DayTime& day_time, Actuator& ctrl) {
    if (day_time.is_in_interval(config.plan_vent_start * 60, config.plan_vent_end * 60)) {
        ctrl.request_enable();
    }
}

void control_light(Climate& config, LightSensor& sensor, DayTime& day_time, Actuator& ctrl) {
