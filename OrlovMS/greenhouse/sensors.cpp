...
Termometer::Termometer(const DHT11& sensor) : m_sensor(sensor)
{
}
...
{
    m_temp = m_sensor.readTemperature();
}
...
Hygrometer::Hygrometer(const DHT11& sensor) : m_sensor(sensor)
{
}
...
SoilHygrometer::SoilHygrometer(unsigned int pin) : m_pin(pin)
{
    pinMode(m_pin, INPUT);
}
...
LightSensor::LightSensor(const unsigned int pin) : m_pin(pin)
{
    pinMode(m_pin, INPUT);
}
...
unsigned long DayTime::get_millis()
{
    return m_millis;
}
