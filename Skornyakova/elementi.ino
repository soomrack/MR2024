...
#define DHTPIN 12 //переписать все дифы на капс 
...
class Time {
    ...
    public:
        Time(int h, int m, int s) : hours(h), minutes(m), seconds(s), time_of_day(0), previousMillis(0) {
...
// Датчик освещенности
int Illumination_sensor(Climate &climate){
  if(climate.illumination_min < 30)
  return 1;
  else return 0;
}
...
void Lamp_switch(const boolean lamp_flag){
 ...
 void Illumination_control(Time &mytime,Climate &climate){
  ...
  Lamp_switch(Illumination_sensor(climate));
}


void Humidity_soil_control(Climate &climate){
  static unsigned long int Hum_Time = 0;
  if (Soil_humidity_sensor() > climate.humidity_soil_min and FLAG_POMP_SWITCH == false) 
  {
    Pomp_switch(true);
...
void temperature_control(Climate &climate){
  ...
  if (temp < climate.temperature_min  and FLAG_POMP_SWITCH == false){ 
    Heating_switch(true);
