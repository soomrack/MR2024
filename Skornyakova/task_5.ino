...
void do_lamp(Lamp lamp)
{
  ...
  Ventilator ventilator_array[]= {Ventilator(FAN_PIN_1),Ventilator(FAN_PIN_2)};
  Light_sensor light_sensor_array[]= {Light_sensor(ILLUMINATION_PIN_1), Light_sensor(ILLUMINATION_PIN_2), Light_sensor(ILLUMINATION_PIN_3)};
  Heater heaters_array[] = {Heater(NAGREV_PIN_1), Heater(NAGREV_PIN_2)};
  
  Heaters myheaters(2,heaters_array);
  Ventilators myventilators(2,ventilator_array);
  Light_sensors mylight_sensors(2,light_sensor_array);
  Time Mytime(14, 30, 0);
...
void loop() {
  ...
  Illumination_control(Mytime.time_of_day,climate_tomato,mylight_sensors,mylamp);
  humidity_soil_control(myhumidity_soil_sensor,climate_tomato,mypump);
  temperature_control(mythermometer,climate_tomato,mypump,myheaters,myventilators);
  humidity_control(myhumidity_sensor,climate_tomato,myventilators,myheaters);
